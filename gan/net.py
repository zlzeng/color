...
def gen(inputs):
	with tf.variable_scope('gen', reuse=tf.AUTO_REUSE) as sc:						  # design the nn architecture for the depth network
...
def dis(inputs):
	with tf.variable_scope('dis', reuse=tf.AUTO_REUSE) as sc:						  # design the nn architecture for the depth network
		end_points_collection = sc.original_name_scope + '_end_points'
		with slim.arg_scope([slim.conv2d, slim.conv2d_transpose],	   #define a conv2d operator with fixed params shown below
							normalizer_fn=None,
							weights_regularizer=slim.l2_regularizer(0.05), # using l2 regularizer with 0.05 weight
			...
			with tf.variable_scope('fc'):
				...
				flatten = slim.flatten(cnv6b, scope='flatten')
				legend_pred = slim.fully_connected(flatten, 1, scope='pred',
					normalizer_fn=None, activation_fn=None)

			end_points = utils.convert_collection_to_dict(end_points_collection)
			return legend_pred, end_points	


# unit test of the tensor shape at each layer
# if __name__ == '__main__':
	# logits, end_points = gen(tf.placeholder(shape=[1,512,1024,3], dtype=tf.float32))
	...
	# logits, end_points = dis(tf.placeholder(shape=[1,552,1024,3], dtype=tf.float32))
	# keys = end_points.keys()
	# for k in keys:
	# 	print 'tensor name = {}, shape = {}'.format(k, end_points[k].shape)
	# print 'predict logits shape = ', logits.shape
	# var = [v for v in tf.trainable_variables() if 'dis' in v.name]
	# for v in var:
	# 	print v.name